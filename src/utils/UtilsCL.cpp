...
std::vector<cl_dev_info> UtilsCL::getAllDevices (void) {
    ...
    for (unsigned i = 0; i < ret_num_platforms; i++) {
        ...
        if (ret_num_devices == 0) {
            ...
            CL_SAFE_CALL (clGetDeviceIDs (platforms[i], CL_DEVICE_TYPE_ALL, ret_num_devices, devices, &ret_num_devices) );
...
std::vector<deviceInfo> UtilsCL::listDevices (void) {
    ...
    for (unsigned i = 0; i < available_devices.size(); i++) {
        ...
        temp.type = getFirstWord (available_devices[i].platform_string) + string (" OpenCL");
...
cl_dev_info UtilsCL::getDevice (cl_device_id device) {
    cl_dev_info found_device;
    cl_int ret;
...
void UtilsCL::devInfo (cl_device_id device, int extended) {
    char device_string[BUFFER_STRING_LENGTH];
    cl_bool buf_bool;
    cl_int ret;
    // CL_DEVICE_NAME
    ...
    CL_SAFE_CALL (clGetDeviceInfo (device, CL_DEVICE_QUEUE_PROPERTIES, sizeof (queue_properties), &queue_properties, NULL) );
    ...
    CL_SAFE_CALL (clGetDeviceInfo (device, CL_DEVICE_IMAGE2D_MAX_WIDTH, sizeof (size_t), &szMaxDims[0], NULL) );
...
const char* UtilsCL::oclErrorString (cl_int error) {
...
void UtilsCL::checkError (const cl_int ciErrNum, const char* const operation) {
