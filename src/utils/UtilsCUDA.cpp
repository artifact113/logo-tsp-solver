...
vector<deviceInfo> UtilsCUDA::listDevices(void) {
    ...
    for (unsigned i = 0; i < available_devices.size(); i++) {
        ...
        sstm << available_devices[i].device_string << " (" << available_devices[i].MPs << " x " << available_devices[i].coresPerMP <<
             " Cores @ " << available_devices[i].clockRate << " GHz, " << available_devices[i].global_mem_size <<" MB)";
        temp.name = sstm.str();
        temp.localNum = i;
...
inline int ConvertSMVer2Cores(int major, int minor) {
    // Defines for CUDA Architecture types (using the SM version to determine the # of cores per SM
    typedef struct {
...
vector<cuda_dev_info> UtilsCUDA::getCUDAInfo(int dev, int extended) {
    ...
    if (dev == -1 ) {
        ...
        if (extended) {
            ...
            trace("  Total amount of global memory:                 %.0f MBytes (%llu bytes)\n",
                  (float)deviceProp.totalGlobalMem/1048576.0f, (unsigned long long) deviceProp.totalGlobalMem);

#if CUDART_VERSION >= 2000
            trace("  (%2d) Multiprocessors x (%3d) CUDA Cores/MP:    %d CUDA Cores\n",
                  deviceProp.multiProcessorCount,
                  ConvertSMVer2Cores(deviceProp.major, deviceProp.minor),
                  ConvertSMVer2Cores(deviceProp.major, deviceProp.minor) * deviceProp.multiProcessorCount);

#endif
            trace("  CUDA Clock rate:                                %.0f MHz (%0.2f GHz)\n", deviceProp.clockRate * 1e-3f, deviceProp.clockRate * 1e-6f);
            ...
            trace("  Max Texture Dimension Size (x,y,z)             1D=(%d), 2D=(%d,%d), 3D=(%d,%d,%d)\n",
                  deviceProp.maxTexture1D, deviceProp.maxTexture2D[0], deviceProp.maxTexture2D[1],
                  deviceProp.maxTexture3D[0], deviceProp.maxTexture3D[1], deviceProp.maxTexture3D[2]);
            trace("  Max Layered Texture Size (dim) x layers        1D=(%d) x %d, 2D=(%d,%d) x %d\n",
...
#if CUDART_VERSION >= 4000
            trace("  Concurrent copy and execution:                 %s with %d copy engine(s)\n", (deviceProp.deviceOverlap ? "Yes" : "No"), deviceProp.asyncEngineCount);
#else
...
void UtilsCUDA::CudaTest(const char *msg) {
    cudaError_t e;
