...
__global__ void kernel2opt (city_coords* device_coords, int _cities, ROUTE_DATA_TYPE* device_route, unsigned long number_of_2opts, unsigned int iterations, int* mutex, struct best2_out* best_2opt) {
#else
__launch_bounds__ (1024, 32)
__global__ void kernel2opt (city_coords* device_coords, int _cities, unsigned long number_of_2opts, unsigned int iterations, int* mutex, struct best2_out* best_2opt) {
#endif
...
#pragma unroll
    ...
    for (register int no = 0; no < iter; no++) {
        ...
        if (id < max) {
            ...
            change =
                calculateDistance2DSimple (i, j + 1,   coords) +  calculateDistance2DSimple (i - 1, j, coords)
                - calculateDistance2DSimple (i, i - 1, coords)   -  calculateDistance2DSimple (j + 1, j, coords);
    ...
    //1024 threads
    if (threadIdx.x < 512)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 512].minchange) {
    ...
    //512 threads
    if (threadIdx.x < 256)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 256].minchange) {
    ...
    //256 threads
    if (threadIdx.x < 128)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 128].minchange) {
    ...
    //128 threads
    if (threadIdx.x < 64)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 64].minchange) {
    ...
    //64 threads
    if (threadIdx.x < 32)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 32].minchange) {
    ...
    //32 threads
    if (threadIdx.x < 16)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 16].minchange) {
    ...
    //16 threads
    if (threadIdx.x < 8)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 8].minchange) {
    ...
    //8 threads
    if (threadIdx.x < 4)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 4].minchange) {
    ...
    //4 threads
    if (threadIdx.x < 2)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 2].minchange) {
    ...
    //2 threads
    if (threadIdx.x < 1)
        if (best_values[threadIdx.x].minchange > best_values[threadIdx.x + 1].minchange) {
...
//the subproblem block size is 2048x2048 with 128x128 thread groups iterating 16x16 times (16 times each direction)
#ifndef USE_ORDERED_COORDS
__launch_bounds__ (1024, 64)
__global__ void kernel2opt_extended (city_coords* coords_global, ROUTE_DATA_TYPE* device_route, int _cities, int* mutex, struct best2_out* best_2opt) {
#else
__launch_bounds__ (1024, 64)
__global__ void kernel2opt_extended (city_coords* coords_global, int _cities, int* mutex, struct best2_out* best_2opt) {
#endif
    register int local_id = threadIdx.x + blockIdx.x * blockDim.x;
    register int i, j;
    register int change;
    struct best2_out best;
    register int rA_start, rB_start, rA_end, rB_end;
...
#define OFFSET 2048
    ...
    for (register int a = 1; a < cities; a += OFFSET) {
        ...
        for (register int b = const1; b > a - OFFSET; b -= OFFSET) {
            ...
            for (register int k = rB_start + threadIdx.x ; k < rB_end; k += blockDim.x) {
...
#pragma unroll 16
            ...
            //#endif
            //for a 2048x2048 problem size, each thread has to work 256 times... (16x16)
            //since there are 16K threads running and 4,194,304 exchanges to be checked
