/*
 *   Logo TSP Solver ver. 0.61  Copyright (C) 2012  Kamil Rocki
 *
...
/*
 //mutex lock
 void inline lock( volatile __global int* mutex ) {
 while( atomic_cmpxchg( mutex, 0, 1 ) != 0);
 }

 //mutex unlock
 void inline unlock( volatile __global int* mutex ) {
 atomic_xchg( mutex, 0 );
 }
 */

int inline calculateDistance2DSimple (unsigned int i, unsigned int j,
...
int inline calculate_change (unsigned int i, unsigned int j, __local city_coords* coords) {
    /* Vectorized code*/
...
__kernel void cl2optKernel (	__global const city_coords* dCoords,
