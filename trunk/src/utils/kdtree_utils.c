...
void constructKDTree(city_coords* coords, ROUTE_DATA_TYPE cities) {
    ...
    for (ROUTE_DATA_TYPE i = 0; i<cities; i++) {
        ...
        //check if pointStruct with same coordinate already in vector/tree. If not: insert in vector and tree
        kd_tree::iterator pItr = tree.find_nearest(point,std::numeric_limits<double>::max()).first;
...
//TODO: clean this up a little bit, possibly encapsulate some operations in functions or/and macros
vector<ROUTE_DATA_TYPE> multipleFragment_KD(ROUTE_DATA_TYPE cities, city_coords* coords) {
    ...
    for (ROUTE_DATA_TYPE i = 0; i < cities - 1; i++) {
        ...
        while (1) {
            ...
            if (degree[y] < 2 && y != tail[x]) {
                ...
                pointStruct point_to_be_erased = { {coords[tail[x]].x, coords[tail[x]].y} };
                point_to_be_erased.id = tail[x];
                kd_tree::iterator pItr = tree_copy.find_nearest(point_to_be_erased, std::numeric_limits<double>::max()).first;
            ...
            point.id = x;
            kd_tree::iterator pItr = tree_copy.find_nearest(point, std::numeric_limits<double>::max()).first;

            tree_copy.erase_exact(((struct pointStruct)*(pItr)));

            pItr = tree_copy.find_nearest(point, std::numeric_limits<double>::max()).first;
...
//get the list of the howMany nearest elements to the point (exclude the point itself) - O(logn) - sorted
vector<ROUTE_DATA_TYPE> getNN_KD_list(ROUTE_DATA_TYPE index, city_coords* coords, ROUTE_DATA_TYPE howMany) {
...
vector<ROUTE_DATA_TYPE> initRoute_KD_NN(ROUTE_DATA_TYPE size, city_coords* coords) {
...
ROUTE_DATA_TYPE getNN_KD(ROUTE_DATA_TYPE index, city_coords* coords) {
...
void removePointKD(city_coords* coords, ROUTE_DATA_TYPE index) {
...
void removePointsKDTree(city_coords* coords, vector<ROUTE_DATA_TYPE> &points) {
    ...
    while (vector_with_points.size() > 0) { //delete all pointStructs from tree which are in the vector
        ...
        /*    if (find(vector_with_points.begin(),vector_with_points.end(),element_to_erase) == vector_with_points.end())
         {
         kd_tree::iterator not_there = tree.find(element_to_erase);
         if (not_there != tree.end())
         {
