...
void run (cmdArguments* args) {
    ...
    if (coords == NULL) {
        fprintf (stderr, "go:coords:Could not allocate that much memory (%ld B)", sizeof (city_coords) * MAX_CITIES);
        exit (-1);
...
vector<ROUTE_DATA_TYPE> optimize (cmdArguments* args, city_coords* coords, vector<ROUTE_DATA_TYPE> route, unsigned short method, int once) {
    ILSGlobalSolver *solver = NULL;
    vector<deviceInfo> selectedDevices = selectDevices (args, method);

    if (method == MODE_CPU) {
        solver = new ILSGlobalSolverSequential (coords, args, getCPUInfo(), TYPE_GLOBAL_SOLVER_CPU, route.size(), selectedDevices[0].localNum,
                                                args->solution, args->timelimit, args->error, args->showLocalOptimizationInfo);

    } else if (method == MODE_CPU_PARALLEL) {
        std::stringstream sstm;
        sstm << args->maxCoresUsed << " x " << getCPUInfo();
        solver = new ILSGlobalSolverSequential (coords, args, sstm.str(), TYPE_GLOBAL_SOLVER_CPU_PARALLEL, route.size(), selectedDevices[0].localNum,
                                                args->solution, args->timelimit, args->error, args->showLocalOptimizationInfo);

    } else if (method == MODE_CUDA) {
        solver = new ILSGlobalSolverSequential (coords, args, "CUDA| ", TYPE_GLOBAL_SOLVER_CUDA, route.size(), selectedDevices[0].localNum,
                                                args->solution, args->timelimit, args->error, args->showLocalOptimizationInfo);

    } else if (method == MODE_OPENCL) {
        solver = new ILSGlobalSolverSequential (coords, args, "OPENCL| ", TYPE_GLOBAL_SOLVER_CL, route.size(), selectedDevices[0].localNum,
                                                args->solution, args->timelimit, args->error, args->showLocalOptimizationInfo);
    }
...
vector<ROUTE_DATA_TYPE> optimizeThreaded (cmdArguments* args, city_coords* coords, vector<ROUTE_DATA_TYPE> route, unsigned short method) {
    int ret;
    ...
    for (unsigned int i = 0; i < selectedDevices.size(); i++) {
        ...
        ret = pthread_create ( &threads[i], NULL, optimizationThread, (void*) &messages[i]);
...
void *optimizationThread (void* data) {
    ...
    if (method == MODE_CPU) {
        sstm  << getCPUInfo();
        solver = new ILSGlobalSolverMT (coords, args, getCPUInfo(), TYPE_GLOBAL_SOLVER_CPU, route.size(), dev, id,
                                        args->solution, args->timelimit, args->error, args->showLocalOptimizationInfo,
                                        args->pthreads, args->setAffinity, args->maxCoresUsed, args->comm);

    } else if (method == MODE_CPU_PARALLEL) {
        std::stringstream sstm;
        sstm << args->maxCoresUsed << " x " << getCPUInfo();
        solver = new ILSGlobalSolverMT (coords, args, sstm.str(), TYPE_GLOBAL_SOLVER_CPU_PARALLEL, route.size(), dev, id,
                                        args->solution, args->timelimit, args->error, args->showLocalOptimizationInfo,
                                        args->pthreads, args->setAffinity, args->maxCoresUsed, args->comm);

    } else if (method == MODE_CUDA) {
        sstm << "CUDA| ";
        solver = new ILSGlobalSolverMT (coords, args, sstm.str(), TYPE_GLOBAL_SOLVER_CUDA, route.size(), dev, id,
                                        args->solution, args->timelimit, args->error, args->showLocalOptimizationInfo,
                                        args->pthreads, args->setAffinity, args->maxCoresUsed, args->comm);

    } else if (method == MODE_OPENCL) {
        sstm << "OpenCL| ";
        solver = new ILSGlobalSolverMT (coords, args, sstm.str(), TYPE_GLOBAL_SOLVER_CL, route.size(), dev, id,
                                        args->solution, args->timelimit, args->error, args->showLocalOptimizationInfo,
                                        args->pthreads, args->setAffinity, args->maxCoresUsed, args->comm);
    }
...
void benchmark (cmdArguments* args, city_coords* coords, vector<ROUTE_DATA_TYPE> route, unsigned short method, int BENCHMARK_ITERS) {
    ILSGlobalSolver *solver = NULL;
    vector<deviceInfo> selectedDevices = selectDevices (args, method);
    ...
    //estimate the FLOPs...
    if (method == TYPE_GLOBAL_SOLVER_CPU || method == TYPE_GLOBAL_SOLVER_CPU_PARALLEL || method == TYPE_GLOBAL_SOLVER_CPU_PARALLEL_HYBRID) {
        totalFlops = (3 * CPU_FLOPS_PER_DISTANCE * total2opts) / 2 + CPU_FLOPS_PER_DISTANCE * (route.size() - 3);
    ...
    if (solver != NULL) {
        for (int i = 0; i < BENCHMARK_ITERS; i++) {
            ...
            trace ("Total Time = %.6f s (HtD = %.6f s, kernel = %.6f s, DtH = %.6f s)\n", T.HtD_time + T.kernel_time + T.DtH_time, T.HtD_time, T.kernel_time, T.DtH_time);
            trace ("GFLOPS = %.2f\n", (double) totalFlops / (double) (1000.0 * 1000.0 * 1000.0 * (T.kernel_time) ) );
            trace ("2-opt checks/s = %.2f M\n", (double) total2opts / (double) (1000.0 * 1000.0 * (T.kernel_time) ) );
        }
...
vector<deviceInfo> selectDevices (cmdArguments* args, unsigned short method) {
    ...
    if (method == MODE_CPU || method == MODE_CPU_PARALLEL || method == MODE_CPU_ALL || method == MODE_ALL || method == MODE_CPU_PARALLEL_HYBRID) {
        trace ("Querying CPU...\n");
        ...
        if (method == MODE_CPU_PARALLEL || method == MODE_ALL || method == MODE_CPU_PARALLEL_HYBRID) {
            temp.localNum = 0;
            std::stringstream sstm;
    ...
    if (method == MODE_CUDA || method == MODE_ALL) {
        ...
        availableDevices.insert (availableDevices.end(), cudaDevices.begin(), cudaDevices.end() );
    }
    ...
    if (args->autoDevice == 1) {
        for (unsigned t = 0; t < availableDevices.size(); t++ ) {
            trace ("Auto Selected [%2d]: %s [%s, local id = %ld]\n", t, availableDevices[t].name.c_str(),
                   availableDevices[t].type.c_str(), availableDevices[t].localNum);
            selectedDevices.push_back (availableDevices[t]);

            if (selectedDevices.size() >= args->maxDevices && args->maxDevices != -1) break;
        }
        ...
        for (unsigned t = 0; t < tokens.size(); t++) {
            trace ("Selected [%2d]: %s [%s, local id = %ld]\n", tokens[t], availableDevices[tokens[t]].name.c_str(),
                   availableDevices[tokens[t]].type.c_str(), availableDevices[tokens[t]].localNum);
        ...
        trace ("Selected [%2d]: %s [%s, local id = %ld]\n", args->device, availableDevices[args->device].name.c_str(),
               availableDevices[args->device].type.c_str(), availableDevices[args->device].localNum);

        if (method != MODE_CPU_ALL) selectedDevices.push_back (availableDevices[args->device]);
    }
